...
from .layout import ANCHOR_CENTER, VerticalLayout, HorizontalLayout
...
class ColorSelector(Control):
    ...
    def on_mouse_release(self, x, y, button, modifiers):
        ...
        self.slider = Slider(value=self.wheel.alpha, min_value=0.0, max_value=255.0, width=256, on_set=on_alpha_set)
        ...
        self.red_input   = Input(text=str(self.color[0]), on_input=on_red_set, length=3, max_length=3)
        self.green_input = Input(text=str(self.color[1]), on_input=on_green_set, length=3, max_length=3)
        self.blue_input  = Input(text=str(self.color[2]), on_input=on_blue_set, length=3, max_length=3)
